    ...
    <div className="rounded-xl border-2 bg-[var(--card)] overflow-hidden" style={{ borderColor: `${color}40` }}>
      ...
      {expanded && (
        <div className="p-3">
          ...
          {/* 成员 */}
          <div className="mb-3">
            <p className="text-[10px] uppercase tracking-wider text-[var(--text-muted)] mb-2 font-bold">🛡️ 成员</p>
            <div className="flex flex-wrap gap-2">
              {members.map((agent) => (
                <AgentCard key={agent.id} agent={agent} onClick={() => onAgentClick(agent)} />
              ))}
              {members.length === 0 && (
                <p className="text-xs text-[var(--text-muted)] italic">暂无成员</p>
              )}
            </div>
          </div>
      ...
      <div className="flex flex-col gap-3 mb-6 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="px-4 py-2 rounded-lg bg-[var(--accent)] text-[var(--bg)] text-sm font-bold hover:opacity-90 transition cursor-pointer"
          >
            + 新建军团
          </button>
          <button
            onClick={() => setShowAddTask(true)}
            className="px-4 py-2 rounded-lg border border-[var(--border)] text-sm font-bold hover:border-[var(--accent)] transition cursor-pointer"
          >
            + 新建任务
          </button>
          <Link
            href="/agent-channel"
            className="px-4 py-2 rounded-lg border border-[var(--border)] text-sm font-bold hover:border-[var(--accent)] transition"
          >
            🤖 Agent频道 →
          </Link>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setSelectedAgent(null)}>
          <div className="w-full max-w-sm bg-[var(--card)] border-2 border-[var(--border)] rounded-xl p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              ...
              className="mt-4 w-full px-4 py-2 rounded-lg border border-[var(--border)] text-sm font-bold hover:border-[var(--accent)] transition"
            >
              关闭
            </button>
          </div>
